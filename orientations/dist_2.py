...
DD = 8.
N = 100 # number of orientations to generate
for i in range(N):
    min_d = 0.8
    while min_d < 0.9:
        origin = read('vc1.xyz')
        ...
        pph4 = read('PPh4.xyz')
        tt = pph4.get_center_of_mass()
        pph4.translate(-1.0*tt)
        pph4.translate([DD, 0.0, 0.0])
        origin.extend(pph4)
        pph4 = read('PPh4.xyz')
        tt = pph4.get_center_of_mass()
        pph4.translate(-1.0*tt)
        pph4.translate([0.0, DD, 0.0])
        origin.extend(pph4)
        dist = origin.get_all_distances()
        for k in range(dist.shape[1]):
            dist[k,k] = 2.0
        min_d = numpy.amin(dist)
        print(min_d)
    filename = str(i) + '.xyz'
    print(filename)
